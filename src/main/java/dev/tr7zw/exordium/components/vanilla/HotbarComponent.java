...
public class HotbarComponent implements BufferComponent<Void> {
    ...
    @Getter
    private static final Identifier id = NMSHelper.getResourceLocation("minecraft", "hotbar");
    ...
    public void captureState(Void context) {
        ...
        if (player == null)
            return;
        for (int m = 0; m < 9; m++) {
            store(player.getInventory().getItem(m), m, player);
        }
    ...
    public boolean hasChanged(Void context) {
        ...
        if (player == null)
            return true;
        if (selectedSlot != player.getInventory().getSelectedSlot()) {
            return true;
        }
        for (int m = 0; m < 9; m++) {
